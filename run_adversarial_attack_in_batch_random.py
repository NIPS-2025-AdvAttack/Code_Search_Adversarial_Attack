...
from attack_scripts.code_search_adv_attack_batch_random import adversarial_attack_random
...
if __name__ == "__main__":
    ...
    if args.max_iter != 1:
        raise ValueError("The current version only supports max_iter=1")
    ...
    for attack_count in range(attack_iter):
        for i in range(len(test_qids)):
            ...
            for j in range(0, len(test_cids), batch_size):
                ...
                attack_records = adversarial_attack_random(
                    query_texts,
                    code_texts,
                    model,
                    tokenizer,
