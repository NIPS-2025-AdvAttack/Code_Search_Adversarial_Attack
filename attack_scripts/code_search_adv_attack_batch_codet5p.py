...
def token_replace_options_batch(query_texts, code_texts, model, tokenizer, device=torch.device('cuda:0'), max_length=512):
    ...
    code_init_emb_grad, code_original_toks_init_emb = grad_with_respect_to_code_init_emb_batch(query_texts, code_texts, model, tokenizer, device, max_length)
    ...
    all_possible_embeddings = model.get_input_embeddings().weight.data.cpu()
...
def adversarial_attack_batch(query_texts, code_texts, model, tokenizer, device=torch.device('cuda:0'), max_length=512, pl='python'):
